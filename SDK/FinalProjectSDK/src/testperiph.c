...
struct LED_STRIP
{
	volatile u32 * led_add;
	volatile u32 * led_value;
	volatile u32 * print_leds;
	volatile u32 * busy_writing;
...
int main() 
{
   ...
   leds.led_add =      slv_regs;
   leds.led_value =    slv_regs+1;
   leds.print_leds =   slv_regs+2;
   leds.busy_writing = slv_regs+3;
   ...
   int k=0;
	while(1)
	{
		...
		u32 cmd;
			...
			{
				*(leds.led_add) = i;
				for (j = 0; j <10;j++); // spin for a while
				if ((k+i)%3 == 0)
					*(leds.led_value) = 0x110000;
				else if ((k+i)%3 == 1)
					*(leds.led_value) = 0x001100;
				else if ((k+i)%3 == 2)
					*(leds.led_value) = 0x000011;
				for (j = 0; j <10;j++); // spin for a while

			}
			while (*(leds.busy_writing)==1);
			*(leds.print_leds) = 1;
			//for (j = 0; j <10;j++);
			*(leds.print_leds) = 0;
			sleep_in_ms(100);
...
//		case 1:	single_pixel_testing();								break;
