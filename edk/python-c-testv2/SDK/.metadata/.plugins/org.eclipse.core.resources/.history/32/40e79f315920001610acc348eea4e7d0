...
//					pixel_buffer[i].blue  = byte_received[i*3+4];
			...
			case 2:
				for (i = 80; i < PIXELS; i++)  // 80 to 142 (63 pixels)
				{
					pixel_buffer[i].red   = byte_received[i*3+2];
					pixel_buffer[i].green = byte_received[i*3+3];
					pixel_buffer[i].blue  = byte_received[i*3+4];
...
void receive_bytes_of_data(void)
{
	...
	else
	{
		...
		for (i = 1 ; i < count ; i++)
		{
			...
			if (i <= count - 3) crc_value = crc_ccitt(i,i ,crc_value );
		...
		{
			crc_c.crc_u8[0] = byte_received[count - 1];
			crc_c.crc_u8[1] = byte_received[count - 2];
