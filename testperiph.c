...
volatile u32 * const slv_regs 	= (volatile u32 *) XPAR_LED_BRAM_DRIVER_0_BASEADDR;
...
//      PORT MAP (
//        pixel00   : in  std_logic_vector(23 downto 0);  // slv_regs[0]
//        pixel01   : in  std_logic_vector(23 downto 0);  // slv_regs[1]
//        pixel02   : in  std_logic_vector(23 downto 0);  // slv_regs[2]
//        pixel03   : in  std_logic_vector(23 downto 0);  // slv_regs[3]
//        pixel04   : in  std_logic_vector(23 downto 0);  // slv_regs[4]
//        pixel05   : in  std_logic_vector(23 downto 0);  // slv_regs[5]
//        pixel06   : in  std_logic_vector(23 downto 0);  // slv_regs[6]
//        pixel07   : in  std_logic_vector(23 downto 0);  // slv_regs[7]
//        pixel08   : in  std_logic_vector(23 downto 0);  // slv_regs[8]
//        pixel09   : in  std_logic_vector(23 downto 0);  // slv_regs[9]
//        pixel10   : in  std_logic_vector(23 downto 0);  // slv_regs[10]
//        pixel11   : in  std_logic_vector(23 downto 0);  // slv_regs[11]
//        pixel12   : in  std_logic_vector(23 downto 0);  // slv_regs[12]
//        transmit  : out std_logic  // slv_regs[13]
//	    );
...
int main() 
{
	...
	while(1)
	{
		if (transmit)
		{
			for (int j,i = 0; i < PIXELS; i++)
			{
				while (*(my_driver->busy) != 0);
				
				*(my_driver->pixel_addr) = i;
//				*(my_driver->pixel_data) = pixel_buffer[i][0];
			...
			*(my_driver->pixel_addr) = NULL_ADDR;
		}
		else
		{
			...
			switch( cmd )
			{
				...
				case 1:
					*(my_driver->print_leds) = 1;
					for (i = 0; i < 84; i++)    // 1 to 84 (84 pixels)
					{
//						pixel_buffer[i][0] = byte_received[i*3+2];
//						pixel_buffer[i][1] = byte_received[i*3+3];
//						pixel_buffer[i][2] = byte_received[i*3+4];
						pixel_buffer[i] = { byte_received[i*3+2],   // red
						                    byte_received[i*3+3],   // green
						                    byte_received[i*3+4] }  // blue
					}
					break;
					*(my_driver->print_leds) = 0;
				// RECEIVE Batch #2 (Batch #1 is next)
				case 2:
					for (i = 84; i < PIXELS; i++)  // 85 to 143 (59 pixels)
					{
//						pixel_buffer[i][0] = byte_received[i*3+2];
//						pixel_buffer[i][1] = byte_received[i*3+3];
//						pixel_buffer[i][2] = byte_received[i*3+4];
